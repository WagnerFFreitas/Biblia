// --- START OF FILE nvi.js ---

/**
 * Arquivo: nvi.js
 * Descrição: Script específico para a versão NVI (Nova Versão Internacional) da Bíblia
 * Este arquivo contém as funções necessárias para carregar e exibir versículos da versão NVI,
...
window.NOME_VERSAO_COMPLETA_BIBLIA = 'Nova Versão Internacional';
...
/**
 * Carrega e exibe um versículo específico da Bíblia NVI
 * @param {string} livro - Nome do livro bíblico (chave, ex: "genesis")
 * @param {number} capitulo - Número do capítulo
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        if (data.versiculos && data.versiculos[versiculo]) {
            // A NVI provavelmente tem títulos de seção internos.
            // Se os seus arquivos JSON para NVI incluírem 'titulos', eles serão usados.
        ...
        console.error(`[NVI] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NVI):`, error);
        const textoP = document.createElement('p');
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        const data = await response.json();
        // A lógica para títulos internos é mantida, caso seus dados os incluam para NVI.
        return data.titulos && data.titulos[versiculo] ? data.titulos[versiculo] : null;
    } catch (error) {
        console.error(`[NVI] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (NVI):`, error);
        return null;
    }
};
