// --- START OF FILE nvt.js ---

/**
 * Arquivo: nvt.js
 * Descrição: Script específico para a versão NVT (Nova Versão Transformadora) da Bíblia
 * Este arquivo contém as funções necessárias para carregar e exibir versículos da versão NVT,
...
window.BIBLE_VERSION_FULL_NAME = 'Nova Versão Transformadora';
...
/**
 * Carrega e exibe um versículo específico da Bíblia NVT
 * @param {string} livro - Nome do livro bíblico (chave, ex: "genesis")
 * @param {number} capitulo - Número do capítulo
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        // Caminho alterado para a versão NVT
        ...
        if (data.versiculos && data.versiculos[versiculo]) {
            // A NVT provavelmente tem títulos de seção internos.
            // Se os seus arquivos JSON para NVT incluírem 'titulos', eles serão usados.
        ...
        console.error(`[NVT] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NVT):`, error);
        const textoP = document.createElement('p');
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {
    ...
    console.log(`[NVT] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);
    try {
        // Caminho alterado para a versão NVT
        const response = await fetch(`../version/nvt/${livro}/${capitulo}.json`);
        if (!response.ok) {
        ...
        const data = await response.json();
        // A lógica para títulos internos é mantida, caso seus dados os incluam para NVT.
        return data.titulos && data.titulos[versiculo] ? data.titulos[versiculo] : null;
    } catch (error) {
        console.error(`[NVT] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (NVT):`, error);
        return null;
    }
};
