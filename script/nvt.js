...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    if (!content) {
        console.error("[NVT] Elemento .content não encontrado.");                // Loga erro se não encontrar o container
        return;
    }
    ...
    try {
        const response = await fetch(`../versao/nvt/${livro}/${capitulo}.json`);  // Caminho para o JSON do capítulo
        if (!response.ok) {
