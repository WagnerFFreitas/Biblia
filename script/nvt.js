// --- START OF FILE script/nvt.js ---

window.BIBLE_VERSION = 'nvt';
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NVT.`);

// --- Dados Específicos de Contagem de Versículos (NVT) ---
// Esta função será chamada por biblia-navegacao.js
window.getSpecificVerseCount = function(livro, capitulo) {
    // ***** ATENÇÃO MUITO IMPORTANTE: ESTE OBJETO PRECISA SER PREENCHIDO COM OS DADOS CORRETOS DE CONTAGEM DE VERSÍCULOS PARA A VERSÃO NVT! *****
    // ***** OS DADOS ABAIXO SÃO APENAS UM MODELO E PROVAVELMENTE NÃO CORRESPONDEM À NVT. *****
    ...
    const versiculosPorCapitulo = {
        ...
        "salmos": { 1:6, 2:12, 150:6 }, // NVT Salmos, verificar contagem exata.
        "proverbios": { 1: 33, 2: 22, 31:31 },
        "eclesiastes": { 1: 18, 12:14 },
        "cantares": { 1: 17, 2: 17, 3: 11, 4: 16, 5: 16, 6: 13, 7: 13, 8: 14 }, // Ou "Cântico dos Cânticos"
    ...
    // ***** FIM DOS DADOS NVT (VERIFICAR E ATUALIZAR COMPLETAMENTE!) *****

    const contagem = versiculosPorCapitulo[livro]?.[capitulo];
    if (typeof contagem === 'undefined') {
        console.warn(`[NVT] Contagem de versículos não encontrada para ${livro} ${capitulo}. POR FAVOR, VERIFIQUE OS DADOS NO ARQUIVO nvt.js!`);
        return 0;
    }
    return contagem;
};

// --- Função Específica para Carregar Versículo (NVT - JSON) ---
// Esta função será chamada por biblia-navegacao.js
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    console.log(`[NVT] Carregando: ${livro} ${capitulo}:${versiculo}`);
    const content = document.querySelector('.content');
    if (!content) {
        console.error("[NVT] Elemento .content não encontrado.");
        return;
    }
    ...
    try {
        // Caminho específico para NVT JSON
        const response = await fetch(`../version/nvt/${livro}/${capitulo}.json`);
        if (!response.ok) {
        ...
        if (!data.versiculos || typeof data.versiculos[versiculo] === 'undefined') {
            throw new Error(`Texto do versículo ${versiculo} não encontrado no campo 'versiculos' do JSON (NVT).`);
        }
        if (typeof data.versiculos[versiculo] !== 'string') {
             throw new Error(`O conteúdo do versículo ${versiculo} (em 'versiculos.${versiculo}') não é uma string, conforme esperado (NVT).`);
        }
        
        const textoDoVersiculo = data.versiculos[versiculo];

        // NVT, como traduções modernas, pode usar títulos por seções/versículos.
        ...
        console.error(`[NVT] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo}:`, error);
        const textoP = document.createElement('p');
        textoP.textContent = `ERRO NVT: ${error.message}`;
    ...
    if (window.titulo) {
        window.titulo.textContent = `${livro.toUpperCase()} - CAPÍTULO ${capitulo} - VERSÍCULO ${versiculo} (NVT)`;
    } else {
