...
window.getSpecificVerseCount = function(livro, capitulo) {
    ...
    const versiculosPorCapitulo = {
        ...
        "salmos": { 1:6, 2:12, 150:6 }, // NTLH Salmos, verificar contagem exata.
        "proverbios": { 1: 33, 2: 22, 31:31 }, // Placeholder
        "eclesiastes": { 1: 18, 12:14 }, // Placeholder
        ...
        "malaquias": { 1: 14, 2: 17, 3: 18, 4: 6 }, // NTLH pode ter 4 capítulos em Malaquias.
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        if (!data.versiculos || typeof data.versiculos[versiculo] === 'undefined') {
            throw new Error(`Texto do versículo ${versiculo} não encontrado no campo 'versiculos' do JSON (NTLH).`);
        }
        if (typeof data.versiculos[versiculo] !== 'string') {
             throw new Error(`O conteúdo do versículo ${versiculo} (em 'versiculos.${versiculo}') não é uma string, conforme esperado (NTLH).`);
        }
        ...
        console.error(`[NTLH] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo}:`, error);
        const textoP = document.createElement('p');
        textoP.textContent = `ERRO NTLH: ${error.message}`;
