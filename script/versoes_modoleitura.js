...
(function() {
    ...
    window.carregarCapituloModoLeitura = async function(livro, capitulo) {                     // Define a função principal, que é 'assíncrona' para poder aguardar downloads.
        ...
        if (!containerLeitura) {                                                               // Se o container não existe, este bloco o cria.
            ...
            const elementoReferencia = areaConteudoLeitura.querySelector('#dynamic-chapter-buttons-container') || areaConteudoLeitura.querySelector('h2'); // Encontra um ponto de referência para inserir o container.
            ...
            if (ehVersaoHtml) {                                                                // Inicia o bloco de lógica para as versões em formato HTML.
                ...
                doc.querySelectorAll('div[id^="versiculo-"]').forEach(div => {                 // Percorre cada 'div' de versículo dentro do HTML baixado.
                    ...
                    if (match) {                                                               // Se encontrou um número de versículo...
                        ...
                        if (texto) htmlConstruido += `<div class="verse-container"><sup class="verse-number">${numero}</sup><span class="verse-text">${texto}</span></div>`; // Monta e adiciona o HTML do versículo.
                    }
                });
            ...
            // Este bloco executa a lógica para carregar e processar versões em formato JSON.
                ...
                for (let i = 1; i <= versiculos.length; i++) {                                 // Percorre todos os versículos em ordem.
                    ...
                    if (dadosCapitulo.versiculos[chave]) {                                     // Verifica se o versículo existe.
                        if (dadosCapitulo.titulos && dadosCapitulo.titulos[chave]) htmlVersiculos += `<h3 class="verse-section-title">${dadosCapitulo.titulos[chave]}</h3>`; // Adiciona títulos de seção, se existirem.
                        htmlVersiculos += `<div class="verse-container"><sup class="verse-number">${i}</sup><span class="verse-text">${dadosCapitulo.versiculos[chave]}</span></div>`; // Monta e adiciona o HTML do versículo.
                    }
                }
    ...
    window.toggleReadingMode = async function(ativar, livro, capitulo) {                       // Define a função que liga e desliga o modo leitura.
        ...
        }                                                                                      // Finaliza a verificação do botão.
        ...
        if (ativar) {                                                                          // Inicia a lógica que será executada ao ATIVAR o modo leitura.
            ...
            window.ultimoVersiculoSelecionado = (window.activeVersiculoButton && window.activeVersiculoButton.dataset.versiculo) ? parseInt(window.activeVersiculoButton.dataset.versiculo) : 1; // Salva o versículo atual.
            ...
            if (window.ultimoLivroSelecionado && window.ultimoCapituloSelecionado) {           // Se houver um livro e capítulo selecionados...
                ...
                containerLeitura.innerHTML = '<div class="reading-mode-message" style="text-align:center; padding: 20px;"><p>Por favor, selecione um livro e capítulo primeiro.</p></div>'; // Exibe a mensagem de ajuda.
