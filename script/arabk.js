...
async function loadVersiculo(livro, capitulo, versiculo) {
    try {
        ...
        console.error("Erro ao carregar versículo do JSON:", error);
        const content = document.querySelector('.content');
        const errorMsg = document.createElement('div');
        errorMsg.classList.add('versiculo', 'versiculo-texto');
        errorMsg.textContent = 'Erro ao carregar versículo.';
...
async function createVersiculosButtons(livro, capitulo) {
    ...
    try {
        ...
        const versiculoKeys = Object.keys(versiculos);
        versiculoKeys.forEach(key => {
            const button = document.createElement('button');
            button.textContent = key;
            button.classList.add('botao-versiculo');
            button.addEventListener('click', () => {
                toggleVersiculoText(livro, capitulo, parseInt(key), button);
            });
            versiculosContainer.appendChild(button);
        });
    } catch (error) {
        console.error('Erro ao carregar versículos para criar botões:', error);
        const errorMsg = document.createElement('div');
        errorMsg.textContent = 'Erro ao carregar versículos.';
        versiculosContainer.appendChild(errorMsg);
    }
