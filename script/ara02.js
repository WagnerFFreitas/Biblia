...
// O bloco abaixo cria a função para exibir o texto "SOBRE"
function showSobre() {
    const content = document.querySelector('.content');
    
    // O trecho abaixo remove qualquer conteúdo existente
    const elementsToRemove = content.querySelectorAll('h2, .capitulos-container, .versiculo-texto, .book-content, .sobre-content');
    elementsToRemove.forEach(element => element.remove());
    
    // O trecho abaixo cria o elemento para exibir o texto "SOBRE"
    const sobreContent = document.createElement('div');
    sobreContent.classList.add('sobre-content');
    sobreContent.innerHTML = `
        <h2>SOBRE</h2>
        <p>Este é um aplicativo para leitura da Bíblia Sagrada.</p>
        <p>Versão: 1.0.0</p>
...
// O bloco abaixo adiciona evento de clique ao link "SOBRE"
document.querySelector('.sobre-link').addEventListener('click', (event) => {
    event.preventDefault();
    showSobre();
});

// O bloco abaixo adiciona evento de clique ao botão de menu para dispositivos móveis
