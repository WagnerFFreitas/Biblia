...
window.BIBLE_VERSION = 'naa'; // Define a versão atual para NAA
...
window.getSpecificVerseCount = function(livro, capitulo) {
    const versiculosPorCapitulo = {
        ...
        "2corintios": {1:24,2:17,3:18,4:18,5:21,6:18,7:16,8:24,9:15,10:18,11:33,12:21,13:14},
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    if (!content) { console.error("[NAA] Elemento .content não encontrado."); return; }
    ...
    try {
        ...
        if (data.versiculos && data.versiculos[versiculo]) {
            ...
            console.warn(`[NAA] Versículo ${versiculo} não nos dados de ${livro} ${capitulo}.json`);
        }
    } catch (error) {
    ...
    if (tituloH2) {
        ...
        console.warn("[NAA] Elemento H2 principal não encontrado para atualizar título.");
    }
};
