...
/**
 * Arquivo: naa.js
 * Descrição: Script específico para a versão NAA (Nova Almeida Atualizada) da Bíblia
 * Este arquivo contém as funções necessárias para carregar e exibir versículos da versão NAA,
...
window.BIBLE_VERSION_FULL_NAME = 'Nova Almeida Atualizada';
...
/**
 * Carrega e exibe um versículo específico da Bíblia NAA
 * @param {string} livro - Nome do livro bíblico (chave, ex: "genesis")
 * @param {number} capitulo - Número do capítulo
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        console.error(`[NAA] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NAA):`, error);
        const textoP = document.createElement('p');
