// --- START OF FILE script/nvi.js ---

window.BIBLE_VERSION = 'nvi';
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NVI.`);

// --- Dados Específicos de Contagem de Versículos (NVI) ---
// Esta função será chamada por biblia-navegacao.js
window.getSpecificVerseCount = function(livro, capitulo) {
    // ***** ATENÇÃO MUITO IMPORTANTE: ESTE OBJETO PRECISA SER PREENCHIDO COM OS DADOS CORRETOS DE CONTAGEM DE VERSÍCULOS PARA A VERSÃO NVI! *****
    // ***** OS DADOS ABAIXO SÃO APENAS UM MODELO E PROVAVELMENTE NÃO CORRESPONDEM À NVI. *****
    const versiculosPorCapitulo = {
        ...
        "cantares": { 1: 17, 2: 17, 3: 11, 4: 16, 5: 16, 6: 13, 7: 13, 8: 14 }, // NVI tem Cântico dos Cânticos
        ...
        "malaquias": { 1: 14, 2: 17, 3: 18, 4: 6 }, // NVI geralmente divide Malaquias em 4 capítulos. O capítulo 3 da ACF/NAA pode ser dividido em 3 e 4 na NVI.
        ...
        "2timoteo": { 1: 18, 2: 26, 3: 17, 4:22 }, // NVI tem 4 capítulos em 2 Timóteo.
        "tito": { 1: 16, 2: 15, 3: 15 },
        "filemom": { 1: 25 },
        ...
        "2joao": { 1: 13 },
        "3joao": { 1: 15 }, // NVI tem 15 versículos em 3 João.
        "judas": { 1: 25 },
    ...
    // ***** FIM DOS DADOS NVI (VERIFICAR E ATUALIZAR COMPLETAMENTE!) *****

    const contagem = versiculosPorCapitulo[livro]?.[capitulo];
    if (typeof contagem === 'undefined') {
        console.warn(`[NVI] Contagem não encontrada para ${livro} ${capitulo}. VERIFIQUE OS DADOS NO ARQUIVO nvi.js!`);
        return 0;
    }
    return contagem;
};

// --- Função Específica para Carregar Versículo (NVI - JSON) ---
// Esta função será chamada por biblia-navegacao.js
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    if (!content) {
        console.error("[NVI] Elemento .content não encontrado.");
        return;
    }
    ...
    try {
        // Caminho específico para NVI JSON
        ...
        if (!data.versiculos || typeof data.versiculos[versiculo] === 'undefined') {
            throw new Error(`Texto do versículo ${versiculo} não encontrado no campo 'versiculos' do JSON (NVI).`);
        }
        if (typeof data.versiculos[versiculo] !== 'string') {
             throw new Error(`O conteúdo do versículo ${versiculo} (em 'versiculos.${versiculo}') não é uma string, conforme esperado (NVI).`);
        }
        ...
        const textoP = document.createElement('p');
        textoP.textContent = `ERRO NVI: ${error.message}`;
    ...
    if (window.titulo) {
        window.titulo.textContent = `${livro.toUpperCase()} - CAPÍTULO ${capitulo} - VERSÍCULO ${versiculo} (NVI)`;
    } else {
