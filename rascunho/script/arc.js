...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    console.log(`[ARC] Carregando versículo: ${livro} ${capitulo}:${versiculo}`);
    ...
    try {
        ...
        if (versiculoContentElement) {
            // Clona o conteúdo do versículo
            const versiculoClone = versiculoContentElement.cloneNode(true);

            // Extrai o título (<strong>) se existir
            const strongElement = versiculoClone.querySelector('strong');
            if (strongElement) {
                strongElement.classList.add('titulo-versiculo-interno'); // Consistência com JSON
            }

            // Extrai o texto do versículo (excluindo o <strong>)
            let textoVersiculo = '';
            if (strongElement) {
                const tempClone = versiculoClone.cloneNode(true);
                const tempStrong = tempClone.querySelector('strong');
                tempStrong.remove();
                textoVersiculo = tempClone.textContent.trim();
            } else {
                textoVersiculo = versiculoClone.textContent.trim();
            }

            // Adiciona o título, se existir
            if (strongElement) {
                versiculoElementDiv.appendChild(strongElement);
            }

            // Cria e adiciona o parágrafo para o texto do versículo
            if (textoVersiculo) {
                const textoP = document.createElement('p');
                textoP.id = `versiculo-${versiculo}`;
                textoP.textContent = textoVersiculo;
                versiculoElementDiv.appendChild(textoP);
            } else {
                console.warn(`[ARC] Texto do versículo ${versiculo} está vazio em ${livro}/${capitulo}.html`);
            }
        } else {
            const textoP = document.createElement('p');
            textoP.textContent = `Versículo ${versiculo} não encontrado no arquivo HTML.`;
            versiculoElementDiv.appendChild(textoP);
            console.warn(`[ARC] Elemento #versiculo-${versiculo} não encontrado em ${livro}/${capitulo}.html`);
        }
    } catch (error) {
        console.error(`[ARC] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo}:`, error);
        const textoP = document.createElement('p');
    ...
    // Adiciona o contêiner ao conteúdo principal
    ...
    if (window.titulo) {
        ...
        console.warn('[ARC] Elemento título não encontrado para atualizar.');
    }
};
