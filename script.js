...
function loadSobre() {
    ...
    if (!sobreContent) {
        ...
        sobreContent.innerHTML = `
            ...
            <p>O projeto está em desenvolvimento, estando somente os livros até Isaías 5 completos; a opção SLIDE para a exibição dos versículos em um Datashow. Existe também a opção para BAIXAR alguns conteúdos para estudo teológico e por último a opção UTILIDADES com alguns tópicos, incluído uma lista de sites com cursos variados, sendo a grande maioria gratuitos, contudo estão sendo organizados.</p>
        `;
